...
const TbAssignFunc = ({ searchQuery, setSearchQuery, filterData, customFilter, setCustomFilter }) => {
    return (<>
        ...
        <div className="flex items-center gap-4">
            ...
            {searchQuery && filterData && <div className="text-sm tracking-wide"> <span className={`tracking-normal mr-0.5 text-lg font-extrabold ${filterData.length == 0 ? 'text-red-500' : 'text-blue-600'}`}>{filterData.length}</span> Results Found.</div>}
        </div>
        {/* =========== Left Side Functions ============= */}
